...
function _pfIncomeHtml(events) {
  return (events || []).map(row => `<div class="pf-income-row"><span>${escapeHtml(row.date)} · ${escapeHtml({dividend:'배당',interest:'이자',other_income:'기타 수입',fee:'비용'}[row.kind] || row.kind)} · ${escapeHtml(row.stock_code || '포트폴리오')} · ${pfAttributionMoney(row.amount_krw)} ${escapeHtml(row.memo || '')}</span>${row.from_broker ? '<small>NH 수입·입출금 연동</small>' : row.from_receipt ? '<small>배당 수취 연동</small>' : `<button type="button" class="pf-mini-btn" onclick="pfDeleteIncome(${Number(row.id)})" aria-label="${escapeHtml(row.date)} 분류 내역 삭제">삭제</button>`}</div>`).join('') || '<p class="pf-chart-note">선택 기간에 기록한 수입·비용이 없습니다.</p>';
}
