...
// WebSocket tick 이 올 때마다 renderPortfolio() 로 tbody 전체를 교체하면
// 마우스 커서 아래 있는 tr 이 매번 삭제/재생성되면서 :hover transition
// 이 깜빡임처럼 보이고, 갱신된 행에 효과도 안 붙는다. 가격 변경에 영향
// 받는 셀들만 in-place 로 덮고 그 행에 flash-update 클래스를 부착해
// '이 행이 방금 갱신됐다' 를 시각적으로 표시한다. 구조 변경(정렬/추가/
// 삭제/편집) 은 여전히 renderPortfolio() 가 담당.
function updatePortfolioRowQuote(code) {
  ...
  const targetPrice = _computeTargetPrice({ ...item, price }, portfolioItems);
