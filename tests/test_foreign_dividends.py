...
class CacheHelpersTests(TempDbMixin):
    ...
    async def test_manual_overrides_are_not_clobbered_by_auto(self):
        ...
        await foreign_dividends_repo.upsert_foreign_dividend_manual("AAPL", 9999.0, note="임시 override")
    ...
    async def test_manual_second_call_overwrites_previous_manual(self):
        ...
        await foreign_dividends_repo.upsert_foreign_dividend_manual("AAPL", 1000.0, note="v1")
        await foreign_dividends_repo.upsert_foreign_dividend_manual("AAPL", 2000.0, note="v2")
        rows = await foreign_dividends_repo.list_foreign_dividends()
        aapl = next(r for r in rows if r["stock_code"] == "AAPL")
    ...
    async def test_auto_updates_only_yfinance_rows(self):
        ...
        await foreign_dividends_repo.upsert_foreign_dividend_manual("AAPL", 9999.0, note=None)
        ...
        rows = {r["stock_code"]: r for r in await foreign_dividends_repo.list_foreign_dividends()}
    ...
    async def test_delete_removes_row(self):
        await foreign_dividends_repo.upsert_foreign_dividend_manual("AAPL", 1000.0)
        self.assertTrue(await foreign_dividends_repo.delete_foreign_dividend("AAPL"))
        rows = await foreign_dividends_repo.list_foreign_dividends()
        self.assertEqual(rows, [])
        # Removing a non-existent code is a no-op (returns False, doesn't raise).
        self.assertFalse(await foreign_dividends_repo.delete_foreign_dividend("AAPL"))

    async def test_get_single_row(self):
        """포트폴리오 PUT 후 dispatch 로직이 '이미 값 있으면 skip' 판단에
        사용. row 존재 / 부재 / 빈 코드 모두 no-raise."""
        self.assertIsNone(await foreign_dividends_repo.get_foreign_dividend("AAPL"))
        self.assertIsNone(await foreign_dividends_repo.get_foreign_dividend(""))
        self.assertIsNone(await foreign_dividends_repo.get_foreign_dividend(None))
        await foreign_dividends_repo.upsert_foreign_dividends_auto([
            {"stock_code": "AAPL", "dps_native": 0.96, "currency": "USD", "dps_krw": 1320.0},
        ])
        got = await foreign_dividends_repo.get_foreign_dividend("AAPL")
        ...
        await foreign_dividends_repo.upsert_foreign_dividend_manual("AAPL", 9999.0, "override")
        got2 = await foreign_dividends_repo.get_foreign_dividend("AAPL")
    ...
    async def test_manual_order_first_in_list(self):
        ...
        await foreign_dividends_repo.upsert_foreign_dividend_manual("GOOGL", 2000.0)
        rows = await foreign_dividends_repo.list_foreign_dividends()
        self.assertEqual(rows[0]["stock_code"], "GOOGL")
        self.assertEqual(rows[0]["source"], "manual")


class GetTrailingDividendsResolutionTests(TempDbMixin):
    async def test_foreign_ticker_resolves_via_foreign_dividends(self):
        """해외 코드는 market_data 에 없으니 foreign_dividends 에서만 매치."""
        await foreign_dividends_repo.upsert_foreign_dividends_auto([
            {"stock_code": "AAPL", "dps_native": 0.96, "currency": "USD", "dps_krw": 1320.0},
        ])
        dps = await portfolio_repo.get_trailing_dividends(["AAPL"])
    ...
    async def test_manual_override_wins_over_preferred_sheet(self):
        ...
        await foreign_dividends_repo.upsert_foreign_dividend_manual("005935", 9999.0)
    ...
    async def test_etf_override_beats_zero_market_data(self):
        ...
        # 동일하게 0 표시).
        dps1 = await portfolio_repo.get_trailing_dividends(["273130"])
        self.assertEqual(dps1.get("273130"), 0.0)
        # 관리자가 실제 분배금 입력:
        await foreign_dividends_repo.upsert_foreign_dividend_manual("273130", 3500.0, note="채권 ETF 수동")
        dps2 = await portfolio_repo.get_trailing_dividends(["273130"])
    ...
    async def test_market_data_still_wins_over_foreign(self):
        ...
        await db.commit()
        await foreign_dividends_repo.upsert_foreign_dividend_manual("005930", 9999.0)
        dps = await portfolio_repo.get_trailing_dividends(["005930"])
        self.assertEqual(dps.get("005930"), 1444.0)


class AdminEndpointTests(TempDbMixin):
    ...
    async def test_delete_endpoint(self):
        ...
        self.assertTrue(out["ok"])
        self.assertEqual(await foreign_dividends_repo.get_foreign_dividends_count(), 0)
