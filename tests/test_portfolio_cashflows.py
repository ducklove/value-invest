...
class PortfolioCashflowTransactionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_add_cashflow_and_sync_cash_creates_cash_position_atomically(self):
        ...
        cashflows = await cache.get_cashflows("u1")
        cash_item = await cache.get_portfolio_item("u1", "CASH_KRW")
    ...
    async def test_withdrawal_rejects_without_cashflow_write_when_cash_is_short(self):
        with self.assertRaises(cache.CashflowBalanceError):
            await cache.add_cashflow_and_sync_cash(
                "u1",
                "2026-05-28",
                "withdrawal",
                1500,
        ...
        self.assertEqual(await cache.get_cashflows("u1"), [])
        self.assertIsNone(await cache.get_portfolio_item("u1", "CASH_KRW"))
    ...
    async def test_delete_cashflow_and_sync_cash_reverses_cash_position_atomically(self):
        ...
        deleted = await cache.delete_cashflow_and_sync_cash("u1", result["id"])
        cash_item = await cache.get_portfolio_item("u1", "CASH_KRW")
