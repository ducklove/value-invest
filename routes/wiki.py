...
async def _load_dart_review_context(stock_code: str) -> str:
    """Return the cached DART periodic-filing AI review for Q&A grounding.

    The ask endpoint should not spend extra AI/DART calls implicitly.  It
    reuses the review generated by the analysis tab and includes it as a
    ...
    lines: list[str] = []
    report_name = review.get("report_name") or "최근 정기보고서"
    report_date = review.get("report_date") or ""
    lines.append(f"공시 AI 리뷰: {report_name}{f' ({report_date})' if report_date else ''}")

    cards = payload.get("cards") if isinstance(payload, dict) else []
    if isinstance(cards, list) and cards:
        lines.append("핵심 체크카드:")
        for card in cards[:8]:
            if not isinstance(card, dict):
                continue
            label = card.get("label") or "항목"
            value = card.get("value") or "-"
            tone = card.get("tone") or "neutral"
            detail = card.get("detail") or ""
            lines.append(f"- [{tone}] {label}: {value}{f' — {detail}' if detail else ''}")

    for title, key in (
        ("확인할 항목", "watch_items"),
        ("전/이전 보고서 대비 변화", "comparison_notes"),
    ):
    ...
    summary = review.get("review_md") or (
        payload.get("summary_md") if isinstance(payload, dict) else ""
    )
    if summary:
        lines.append("요약 본문:")
        lines.append(_truncate_context(summary, QA_DART_REVIEW_CHARS))

    source_limits = payload.get("source_limits") if isinstance(payload, dict) else ""
    if source_limits:
        lines.append(f"원문 범위/한계: {source_limits}")
...
답변 가이드:
- DART 정기보고서/공시 리뷰가 있으면 가장 우선적인 회사 원문 근거로 사용하고, (공시, 보고서명/일자) 형식으로 인용하세요.
...
async def ask_stock(
    ...
    dart_block = await _load_dart_review_context(stock_code)
    ...
    if dart_block:
        prompt_sections.append(f"## DART 정기보고서/공시 AI 리뷰\n{dart_block}")
    if macro_block:
    ...
    prompt_sections.append(
        ...
        "숫자를 인용할 때는 단위(원·%·배)를 명확히 쓰고, DART 공시/정기보고서 근거는 "
        "(공시, 보고서명/일자), 증권사 리포트 근거는 (증권사, 발행일) 형식으로 표기하세요. "
        "일반적인 금융 지식도 필요하면 활용하되, "
