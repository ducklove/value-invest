...
@router.get("/diag/ws")
async def diag_ws(request: Request):
    """KIS WebSocket 실시간 체결 상태 진단.

    '대덕전자(보통주) 는 실시간 반영되는데 대덕전자1우(우선주) 는 훨씬
    늦게 반영된다' 같은 질문에 답하기 위한 엔드포인트. 각 종목의 최근
    tick 수신 시각을 보면 (a) KIS WS 가 우리 서버로 해당 종목 체결을
    실제로 보내주고 있는지, (b) 마지막 체결이 몇 초/분 전인지 즉시
    판별 가능.

    응답 필드:
      - now: 서버 현재 시각 (epoch seconds)
      - ws_sessions: 활성 브라우저 WS 세션 수
      - kis_subscriptions: 각 세션이 KIS WS 에 구독 중인 (code, tr_id)
        목록. 여기 code 가 있으면 우리 서버가 KIS 에 해당 종목을
        실시간 체결 통보로 요청한 상태.
      - cached_quotes: 종목별로 { price, change_pct, ts, age_s }. ts 는
        서버가 해당 종목의 WS tick 을 마지막으로 받은 시각. age_s 가
        매우 큰 값 (수백 초~) 이면 거래 자체가 뜸한 것.
      - kis_not_cached: KIS 에 구독은 했는데 cache 에 아직 quote 가
        없는 코드 (즉 오늘 한 번도 tick 을 못 받은 종목). 우선주 지연
        이슈 진단에 가장 중요한 필드.
    """
    await _require_admin(request)
    ...
    from routes.ws_quotes import _sessions as _ws_sessions

    now = _time.time()
    cached = kis_ws_manager.get_all_cached_quotes()
    cached_quotes: dict[str, dict] = {}
    for code, q in cached.items():
        ts = q.get("ts")
        cached_quotes[code] = {
            "price": q.get("price"),
            ...
            "ts": ts,
            "age_s": round(now - ts, 2) if ts else None,
        }

    # Active KIS subscriptions per session.
    kis_subs: list[dict] = []
    all_subscribed_codes: set[str] = set()
    for _browser_ws, sess in list(_ws_sessions.items()):
        conn = sess.ws_conn
        if conn is None:
            continue
        pairs = [{"code": c, "tr_id": t} for (c, t) in sorted(conn._current_subs)]
        for c, _t in conn._current_subs:
            all_subscribed_codes.add(c)
        kis_subs.append({
            "slot_id": getattr(sess.key_slot, "slot_id", None) if sess.key_slot else None,
            "is_active": sess.is_active,
            "subscriptions": pairs,
            "requested": dict(conn._requested),  # 카테고리별 요청 목록
        })

    kis_not_cached = sorted(all_subscribed_codes - set(cached_quotes.keys()))

    return {
        "now": round(now, 2),
        "ws_sessions": len(_ws_sessions),
        "kis_subscriptions": kis_subs,
        "cached_quotes_count": len(cached_quotes),
        "cached_quotes": cached_quotes,
        "kis_not_cached": kis_not_cached,
    }
