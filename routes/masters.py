...
* ``POST /api/masters/simulate`` — 사용자의 위험 성향·투자 기간·선호
  자산군(profile)을 받아 전략별 참고용 배분을 계산한다. 입력이 JSON
...
async def simulate(request: Request, payload: dict = Body(default={})):
    ...
    return investment_masters.personalize(profile if isinstance(profile, dict) else {})
