...
- 조회·쓰기 연결을 분리했고, 요청 쓰기는 `transaction()`으로 직렬화한다.
  복합 조회는 `read_snapshot()`으로 기준 시점을 고정한다. 초기 스키마 부트스트랩과
  별도 연결을 사용하는 현금흐름·스냅샷·전체 교체는 각 연결에서 원자성을 보장한다.
- 포트폴리오 공유 상태는 `PfStore`로 통합했다. 최초 URL 화면은 부가 데이터 로딩보다 먼저 연다.
- API 변경 요청의 출처를 검증하고, 부분 시작 실패·종료 시 작업과 연결을 정리한다.
- 핵심 조회 API 10개에 응답 모델을 적용했다. 정산 조회의 SQL은 저장소로 분리했다.
- 심층 분석·가계부·도구 파일 11개는 화면 진입 시 로드한다. 보유종목 초기 로컬 JS
  요청은 52 → 42개, JS 원본 용량은 약 17% 감소했다.
