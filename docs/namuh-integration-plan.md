...
작성: 2026-09-15. 상태: **조사·설계 완료, 구현·운영 활성화 전**.
코드 기준: value-invest `cba357df`, finance-pi `296e593`.
...
- **설계 초기값은 2세션 × 10등록으로 제한한다. 60종목 수신 가능이라고 가정하지 않는다.** 등록은 `(채널, 구독키)` 단위로 예산을 계산하므로 같은 종목의 체결가와 호가는 별도다. 통보 채널 및 외부 프로그램이 쓰는 세션도 함께 고려한다.
