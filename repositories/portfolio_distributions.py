...
@read_snapshot()
async def balances(user: str, account_id: str | None = None) -> list[dict]:
    from repositories import accounts
    default_id = await accounts.get_default_account_id(user) if account_id else None
    received = {row["currency"]: row for row in await dividend_receipts.receipt_totals(user, account_id)}
    db = await get_db()
    rows = await (await db.execute("SELECT currency,amount,result_json FROM portfolio_distributions WHERE google_sub=?", (user,))).fetchall()
    paid = {}
    for row in rows:
        if account_id and (json.loads(row["result_json"]).get("account_id") or default_id) != account_id:
            continue
...
async def _state(user: str, payload: DistributionInput):
    ...
    total = next((row for row in await balances(user, current(user)) if row["currency"] == payload.currency), {})
...
async def record_distribution(user: str, payload: DistributionCreate):
    ...
    fingerprint = _digest(payload.model_dump(mode="json", exclude={"request_id"} | ({"account_id"} if not payload.account_id else set())))
