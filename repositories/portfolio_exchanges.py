...
async def record_exchange(user: str, exchange: ExchangeCreate) -> dict:
    ...
    fingerprint = _digest(exchange.model_dump(mode="json", exclude={"request_id"} | ({"account_id"} if not exchange.account_id else set())))
    async with transaction() as db:
        ...
        for cash, currency, balance in [(source, exchange.from_currency, result["from_after"]), (target, exchange.to_currency, result["to_after"])]:
            await portfolio.save_portfolio_item(user, f"CASH_{currency}", (cash or {}).get("stock_name", f"{currency} 현금"),
                                                balance, (cash or {}).get("avg_price", 1), currency,
                                                avg_price_currency=(cash or {}).get("avg_price_currency", currency))
        result.update(account_id=current(user), request_id=request_id, created_at=now, replayed=False)
        await db.execute(
